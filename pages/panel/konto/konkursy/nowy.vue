...
<template>
    <div>
        ...
        <div v-if="allQuestions.data?.length > 0" class="mt-[28px]">
            <div class="bg-white pt-[28px] pb-[8px] pl-[28px] border-transparent rounded-[24px]">
                ...
                <div class="md:grid md:grid-cols-2 gap-x-[32px]">
                    <div v-for="(question, index) in allQuestions.data" :key="index"
                        class="flex items-center mb-[23px] pb-[10px]  w-full"
                        :class="allQuestions.data.length == index + 1 ? 'border-b border-b-white md:border-b-slate-200' : 'border-b border-b-slate-200'">
...
const fetchQuestions = async (query = route.query) => {
    const res = await axiosInstance.get(`questions/all?${formatQueryString(query)}&per_page=8`);
